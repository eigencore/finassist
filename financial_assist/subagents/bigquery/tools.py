...
- CRUD operations: This kind of query will 
...
def get_user_context(user_id=None):
    """
    Get user context from BigQuery.
    This function retrieves user information and account details from the
    BigQuery database. It constructs a JSON object containing the user's basic
    information and a list of their accounts.
    
    Args:
        user_id (str, optional): ID del usuario. Si no se proporciona, se tomará
                                 el valor de la variable de entorno USER_ID.
    
    Returns:
        str: Contexto del usuario en formato JSON
    """
    # Obtener variables de entorno
    project_id = os.getenv("BQ_PROJECT_ID", "ninth-botany-460322-r5")
    dataset_id = os.getenv("BQ_DATASET_ID", "finassist_db")
    default_user_id = os.getenv("USER_ID", "user_001")
    
    # Usar el user_id proporcionado o el valor predeterminado
    user_id = user_id or default_user_id
    
    logging.info(f"Getting user context for user_id: {user_id}")
    ...
    try:
        ...
        logging.info(f"User context generated successfully for {user_id}")
        return context_json
